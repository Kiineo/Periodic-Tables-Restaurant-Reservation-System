...
/**
 * Fetch `json` from the specified URL and handle error status codes, ignoring `AbortError`.
 *
...
export async function listReservations(params, signal) {
  ...
  return await fetchJson(url, { headers, signal }, []).then(formatReservationDate).then(formatReservationTime);
}
...
export async function updateReservation(updatedReservation, signal) {
  ...
  return await fetchJson(url, options, updatedReservation).then(formatReservationDate).then(formatReservationTime);
}
